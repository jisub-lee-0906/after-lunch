...
def compute_level_targets(sample_rows: int | None) -> dict[str, int] | None:
...
def should_stop_collecting(collected: dict[str, int], targets: dict[str, int] | None) -> bool:
    if targets is None:
        return False
    return all(collected.get(level, 0) >= targets.get(level, 0) for level in targets)


def build_output_paths(config: NationwideRunConfig) -> dict[str, Path]:
    if config.sample_rows is None:
        base = config.root_dir / 'datasets' / str(config.year)
        return {
            'raw': base / f'raw_meals_{config.year}.jsonl',
            'refined': base / f'refined_meals_{config.year}.jsonl',
            'high_confidence': base / f'high_confidence_seed_{config.year}.json',
            'app_seed': base / f'app_seed_deduped_{config.year}.json',
            'report': base / f'run_report_{config.year}.json',
        }
    return build_sample_stage_paths(config)
...
def run_sample_pipeline(config: NationwideRunConfig) -> dict:
    ...
    if config.sample_rows is None:
        school_caps = {'초등학교': 5000, '중학교': 5000, '고등학교': 5000}
    chosen_schools = []
    seen = set()
    ...
    for school in school_rows:
        ...
        if config.sample_rows is not None and all(school_count_by_level[level] >= school_caps[level] for level in school_caps):
            break

    collected_by_level = {k: 0 for k in (targets or {'초등학교': 0, '중학교': 0, '고등학교': 0})}
    raw_rows: list[dict] = []
    ...
    report = {
        ...
        'level_collected': collected_by_level if targets is not None else None,
...
if __name__ == '__main__':
    ...
    sample_rows = None if arg == 'full' else int(arg)
